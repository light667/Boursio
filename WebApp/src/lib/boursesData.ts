 ...
 */
export function isScholarshipExpired(bourse: Record<string, unknown>): boolean {
...
export function getAllBourses(includeExpired: boolean = false): Bourse[] {
  const mapped = (rawBourses as Record<string, unknown>[]).map((raw) => {
    ...
    return {
      ...
      qualite_score: (raw.qualite_score as number) || 50,
  ...
  return mapped.filter((b) => !isScholarshipExpired(b as unknown as Record<string, unknown>));
}
