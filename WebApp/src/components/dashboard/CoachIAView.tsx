  ...
  while (i < lines.length) {
    ...
    if (/^## /.test(line)) {
      nodes.push(
        <h3
          key={i}
          className="font-display font-bold text-base text-foreground mt-3 mb-1 leading-snug"
        >
      ...
      nodes.push(
        <h2
          key={i}
          className="font-display font-bold text-lg text-foreground mt-3 mb-1 leading-snug"
        >
...
export const CoachIAView: React.FC<CoachIAViewProps> = ({ studentProfile }) => {
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      toast.error("Le Coach IA n'a pas pu répondre. Veuillez réessayer.");
    } finally {
      setIsTyping(false);
    }
  };
  ...
  const handleCopy = (id: string, text: string) => {
    ...
    setCopiedId(id);
    toast.success("Conseils du Coach IA copiés !");
