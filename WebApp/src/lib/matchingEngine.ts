...
export function calculateScholarshipMatch(
  ...
  if (!profile) {
    ...
    return {
      score: 50,
      reasons: [
        "Veuillez compléter votre profil étudiant pour afficher votre score de matching réel",
      ],
    };
  }
  ...
  if (isDomainMatch) {
    ...
    if (studentField) {
      reasons.push(
        `Filière d'étude (${profile.studyField}) correspondant aux domaines de la bourse`,
      );
    } else {
...
export function rankBoursesForStudent(bourses: Bourse[], profile: StudentProfile | null): Bourse[] {
  return bourses
    .map((b) => {
