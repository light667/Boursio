...
interface AuthModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (user: User, mode: "login" | "register") => void;
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl border border-primary/20 bg-card p-6 shadow-2xl transition-all sm:p-8">
        ...
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {mode === "login"
              ? "Accédez à vos recommandations de bourses sur-mesure."
              : "Rejoignez la 1ère plateforme IA de bourses d'études."}
          </p>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="mb-4 flex w-full items-center justify-center gap-3 rounded-xl border border-border bg-secondary/80 py-3 text-sm font-medium text-foreground transition-all hover:bg-secondary hover:border-primary/40 focus:outline-none focus:ring-2 focus:ring-primary disabled:opacity-50"
        >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="etudiant@boursio.com"
              className="w-full rounded-xl border border-border bg-input px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-primary to-blue-600 py-3 text-sm font-semibold text-white shadow-glow transition-all hover:opacity-95 focus:outline-none focus:ring-2 focus:ring-primary disabled:opacity-50"
          >
            {loading ? (
